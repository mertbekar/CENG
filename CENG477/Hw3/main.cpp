...
void createVerticesAndTriangles(int width, int height, glm::vec3 vertices[], glm::vec3 triangles[]) {
	glm::vec3 vec;
	int idx = 0;
...
int main(int argc, char * argv[]) {
	...
	up = {0.0, 1.0, 0.0};
	gaze = {0.0, 0.0, 1.0};
	eye = {widthTexture / 2.0, widthTexture / 10.0, (-1.0 / 4.0) * widthTexture};
	...
	createVerticesAndTriangles(widthTexture, heightTexture, vertices, triangles);

	GLuint vbo;
	glGenBuffers(1, &vbo);
	glBindBuffer(GL_ARRAY_BUFFER, vbo);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * bufferSize, &(triangles)[0], GL_STATIC_DRAW);
